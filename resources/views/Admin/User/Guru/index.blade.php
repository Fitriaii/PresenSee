...
<div class="container p-6 mx-auto">
    ...
    <div class="mb-4 bg-white rounded-sm shadow-sm">
        <div class="flex items-center justify-between p-6">
            <div>
                <h1 class="text-xl font-bold text-purple-800 font-heading">Manajemen Guru</h1>
                <p class="mt-1 text-sm text-gray-600">Kelola informasi dan data akun guru</p>
            </div>
            <nav class="flex space-x-1 font-sans text-xs text-gray-500">
                ...
                <span>/</span>
                <span class="text-gray-400">Daftar Guru</span>
            </nav>
        </div>
    </div>
    ...
    <div class="z-50 p-6 mb-6 bg-white rounded-sm shadow-lg font-heading">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <!-- Kanan: Search -->
                <form id="searchForm" method="GET" action="{{ route('guru.index') }}" class="w-full sm:w-auto">
            ...
            <!-- Filter & Controls -->
            <form method="GET" action="{{ route('guru.index') }}" id="filterForm" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('guru.index') }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 hover:border-gray-400 focus:ring-2 focus:ring-gray-500 focus:outline-none transition-all duration-200">
            ...
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="flex flex-col gap-3 text-sm text-gray-600 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                        <span>
                            Menampilkan
                            <span class="font-medium text-gray-900">{{ $userGuru->firstItem() ?? 0 }}–{{ $userGuru->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium text-gray-900">{{ $userGuru->total() }}</span> data
                        </span>
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 bg-green-500 rounded-full"></div>
                            <span>{{ $userGuru->where('is_logged_in', 1)->count() }} Aktif</span>
                            <div class="w-2 h-2 ml-3 bg-red-500 rounded-full"></div>
                            <span>{{ $userGuru->where('is_logged_in', 0)->count() }} Nonaktif</span>
                        </div>
                    </div>
        ...
        <div class="mt-4 overflow-auto rounded-sm shadow-sm">
            <table class="min-w-full bg-white rounded shadow-md">
                ...
                <tbody>
                    @forelse ($userGuru as $g)
                    <tr class="text-sm text-gray-700 border-t hover:bg-gray-50">
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $loop->iteration }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $g->nip }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $g->nama_guru }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $g->user->email }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">
                            @if ($g->user->is_logged_in)
                        ...
                        <td class="px-4 py-3 text-left whitespace-nowrap">
                            <div class="flex space-x-2 justify-left">
                                <!-- Lihat -->
                                <a href="{{ route('guru.show', $g) }}" class="text-blue-600 hover:text-blue-800" title="Detail">
                                ...
                                {{-- Tombol Edit --}}
                                <form action="{{ route('guru.edit', $g) }}" method="GET" class="inline-block">
                                    @csrf
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('guru.destroy', $g) }}" method="POST" id="deleteForm-{{ $g->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                    ...
                    <tr class="text-sm text-gray-500">
                        <td colspan="6" class="px-4 py-3 text-center">Tidak ada data guru yang ditemukan.</td>
                    </tr>
        ...
        <div class="mt-4">
            @include('components.pagination', ['data' => $userGuru])
        </div>
    </div>
...
<script>
    ...
    ['status', 'sort', 'perPage'].forEach(id => {
