...
<div class="container p-6 mx-auto">
    ...
    <div class="mb-4 bg-white rounded-sm shadow-sm">
        <div class="flex items-center justify-between p-6">
            <div>
                <h1 class="text-xl font-bold text-purple-800 font-heading">Kelas</h1>
                <p class="mt-1 text-sm text-gray-600">Kelola informasi dan data kelas</p>
            </div>
    ...
    <div class="z-50 p-6 mb-6 bg-white rounded-sm shadow-lg font-heading">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <!-- Kanan: Search -->
                <form id="searchForm" method="GET" action="{{ route('room.index') }}" class="w-full sm:w-auto">
            ...
            <!-- Filter & Controls -->
            <form method="GET" action="{{ route('room.index') }}" id="filterForm" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    <!-- Tahun Filter -->
                    <div class="space-y-2">
                        <label for="tingkatan_kelas" class="block text-sm font-semibold text-gray-700">Tingkatan Kelas</label>
                        <select name="tingkatan_kelas" id="tingkatan_kelas"
                            class="w-full px-3 py-2.5 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 dark:bg-white dark:text-gray-900 dark:border-gray-300 focus:ring-purple-500/20 focus:border-purple-500">
                            <option value="">Semua Tingkatan</option>
                            <option value="7" {{ request('tingkatan_kelas') == '7' ? 'selected' : '' }}>7 (VII)</option>
                            <option value="8" {{ request('tingkatan_kelas') == '8' ? 'selected' : '' }}>8 (VIII)</option>
                            <option value="9" {{ request('tingkatan_kelas') == '9' ? 'selected' : '' }}>9 (IX)</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="sort" name="sort" class="w-full px-3 py-2.5 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 dark:bg-white dark:text-gray-900 dark:border-gray-300 focus:ring-purple-500/20 focus:border-purple-500">
                            <option value="nama_kelas_asc" {{ request('sort') === 'nama_kelas_asc' ? 'selected' : '' }}>Nama (A-Z)</option>
                            <option value="nama_kelas_desc" {{ request('sort') === 'nama_kelas_desc' ? 'selected' : '' }}>Nama (Z-A)</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('room.index') }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 hover:border-gray-400 focus:ring-2 focus:ring-gray-500 focus:outline-none transition-all duration-200">
            ...
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="flex flex-col gap-3 text-sm text-gray-600 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                        <span>
                            Menampilkan
                            <span class="font-medium text-gray-900">{{ $room->firstItem() ?? 0 }}–{{ $room->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium text-gray-900">{{ $room->total() }}</span> data
                        </span>
                    </div>
                    ...
                    @if ($room->count())
                        <div class="text-gray-500">
                            Terakhir diperbarui:
                            <span class="font-medium">{{ $room->first()->updated_at->diffForHumans() }}</span>
                        </div>
                    @endif
            ...
            <table class="min-w-full bg-white rounded shadow-md">
                <thead>
                    <tr class="text-sm font-semibold text-gray-700 bg-gray-100 font-heading">
                        ...
                        <th class="px-4 py-3 text-left uppercase whitespace-nowrap">Nama Kelas</th>
                        <th class="px-4 py-3 text-left uppercase whitespace-nowrap">Jenis Kelas</th>
                        <th class="px-4 py-3 text-left uppercase whitespace-nowrap">Jumlah Siswa</th>
                        <th class="px-4 py-3 text-left uppercase whitespace-nowrap">Wali Kelas</th>
                ...
                <tbody>
                    ...
                    <tr class="text-sm text-gray-700 border-t hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $k->nama_kelas }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $k->jenis_kelas }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $k->siswa_kelas_count }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $k->guru->nama_guru }}</td>

                        <td class="px-2 py-3 text-left whitespace-nowrap">
                            <div class="flex space-x-2 text-xs justify-left">
                                <!-- Lihat -->
                                <a href="{{ route('room.show', $k) }}" class="text-blue-600 hover:text-blue-800" title="Detail">
                                ...
                                <!-- Edit -->
                                <a href="{{ route('room.edit', $k) }}" class="text-purple-600 hover:text-purple-800" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('room.destroy', $k) }}" method="POST" id="deleteForm-{{ $k->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="text-red-600 hover:text-red-800" title="Hapus" onclick="confirmDelete({{ $k->id }})">
                    ...
                    <tr class="text-sm text-gray-500">
                        <td colspan="6" class="px-4 py-3 text-center">Tidak ada data kelas yang ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-4">
            @include('components.pagination', ['data' => $room])
        </div>
    </div>
</div>
...
<script>
    ...
    ['jenis_kelas', 'tingkatan_kelas', 'sort'].forEach(id => {
