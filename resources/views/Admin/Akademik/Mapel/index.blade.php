...
<div class="container p-6 mx-auto">
    ...
    <div class="mb-4 bg-white rounded-sm shadow-sm">
        <div class="flex items-center justify-between p-6">
            <div>
                <h1 class="text-xl font-bold text-purple-800 font-heading">Mata Pelajaran</h1>
                <p class="mt-1 text-sm text-gray-600">Kelola informasi dan data mata pelajaran</p>
            </div>
    ...
    <div class="z-50 p-6 mb-6 bg-white rounded-sm shadow-lg font-heading">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <!-- Kanan: Search -->
                <form id="searchForm" method="GET" action="{{ route('mapel.index') }}" class="w-full sm:w-auto">
            ...
            <!-- Filter & Controls -->
            <form method="GET" action="{{ route('mapel.index') }}" id="filterForm" class="space-y-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <a href="{{ route('mapel.index') }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 hover:border-gray-400 focus:ring-2 focus:ring-gray-500 focus:outline-none transition-all duration-200">
            ...
            <div class="pt-4 mt-4 border-t border-gray-200">
                <div class="flex flex-col gap-3 text-sm text-gray-600 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                        <span>
                            Menampilkan
                            <span class="font-medium text-gray-900">{{ $mapel->firstItem() ?? 0 }}–{{ $mapel->lastItem() ?? 0 }}</span>
                            dari
                            <span class="font-medium text-gray-900">{{ $mapel->total() }}</span> data
                        </span>
                    </div>
                    ...
                    @if ($mapel->count())
                        <div class="text-gray-500">
                            Terakhir diperbarui:
                            <span class="font-medium">{{ $mapel->first()->updated_at->diffForHumans() }}</span>
                        </div>
                    @endif
            ...
            <table class="min-w-full bg-white rounded shadow-md">
                ...
                <tbody>
                    ...
                    <tr class="text-sm text-gray-700 border-t hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $m->kode_mapel }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $m->nama_mapel }}</td>
                        <td class="px-4 py-3 text-left whitespace-nowrap">{{ $m->guru->nama_guru }}</td>

                        <td class="px-2 py-3 text-left whitespace-nowrap">
                            <div class="flex space-x-2 text-xs justify-left">
                                <!-- Lihat -->
                                <a href="{{ route('mapel.show', $m) }}" class="text-blue-600 hover:text-blue-800" title="Detail">
                                ...
                                <!-- Edit -->
                                <a href="{{ route('mapel.edit', $m) }}" class="text-purple-600 hover:text-purple-800" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                ...
                                <!-- Hapus -->
                                <form action="{{ route('mapel.destroy', $m) }}" method="POST" id="deleteForm-{{ $m->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="text-red-600 hover:text-red-800" title="Hapus" onclick="confirmDelete({{ $m->id }})">
                    ...
                    <tr class="text-sm text-gray-500">
                        <td colspan="5" class="px-4 py-3 text-center">Tidak ada data mata pelajaran yang ditemukan.</td>
                    </tr>
                    @endforelse
